...
data_file_path = f'北京-上海数据集-替换决策.xlsx'
...
output_file_path = f'PPO测试结果-替换决策.xlsx'
updated_test_data.to_excel(output_file_path, index=False)

purchases_data = updated_test_data[updated_test_data['决策动作'] == 1].copy()  # 选出决策动作为1的行

purchases_data['价格差'] = purchases_data['目标与替换最小价格'] - purchases_data['历史最低价格']  # 计算机票价格-历史最低价格的差值
cumulative_sum = purchases_data['价格差'].sum()  # 计算差值列的累积和
average_result = cumulative_sum / len(purchases_data)  # 计算平均值：累积和/样本总数
